...
from fixtures import ThingTypeDictFactory, ThingDictFactory, \
    ...
    ObservationIntegerDictFactory, ActuationDictFactory, ContextAwarenessRuleDictFactory
from test_api_actuation import assert_actuations
from test_api_context_awareness_rule import assert_context_awareness_rules
...
@pytest.mark.parametrize(
    ...
    "test_input_observation, test_input_context_awareness_rule, test_input_actuation", [
        (["ThingType1"],
         ["Location1", "41.2, 2.1"],
         ["Platform1", "Location1"],
...
def test_full_api(api_client, orm_client,
    ...
    # ContextAwarenessRule checks
    ## Create ContextAwarenessRule element
    context_awareness_rule = ContextAwarenessRuleDictFactory(
        name=test_input_context_awareness_rule[0],
        sensor_observed_name=test_input_context_awareness_rule[1],
        operation_type=test_input_context_awareness_rule[2],
        executing=test_input_context_awareness_rule[3],
        value_to_compare_boolean=test_input_context_awareness_rule[3],
    )

    rv = api_client.post("/context-awareness-rules/", json=context_awareness_rule)
    assert rv.status_code == 200, rv.data
    response_content = json.loads(rv.data)["data"]["name"]
    assert response_content == context_awareness_rule["name"]

    ## Check ContextAwarenessRule creation
    rv = api_client.get(f"/context-awareness-rules/{response_content}")
    assert rv.status_code == 200
    response_content = json.loads(rv.data)["data"]
    assert_context_awareness_rules(response_content, context_awareness_rule)

    # Actuation checks
    ## Create Actuation element
    actuation = ActuationDictFactory(observation_id=observation_id,
                                     context_awareness_rule_name=test_input_actuation[1],
